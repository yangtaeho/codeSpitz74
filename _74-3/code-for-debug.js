const jstr = (obj) => JSON.stringify(obj); //toJsonStr
...
const parser = input=>{
    ...
    do{ //재귀함수를 처리 하기 위해 스택 루프 처리
		let text = '';
		console.log('parser start :: cursor :: stack :: stacksLast', cursor, jstr(stack), jstr(stacks[stacks.length-1]));
...
const elementNode = (input, cursor, text, stack, stacks)=>{
    ...
    console.log('========================================================\n');
	console.log('elementNode start,  cursor,  char,  input remain ==> ', cursor, ',  ', char, ',  ', input.substring(cursor));
    ...
    if(char === '<'){ //꺾쇠가 등장하는 순간 텍스트 노드가 탄생한다
        ...
        if (input[cursor++] !== '/') { //슬래시를 인식한 순간 판단 가능함.
            ...
            if(isClose) { 
				...
				console.log('elementNode isClose :: cursor,  name,  input[cursor] ==> ', cursor, ',  ', name, ',  ', input[cursor]);
			}
            ...
            if(!isClose){
				...
				console.log('elementNode !isClose :: cursor, stacksLast ==> ', cursor, ',  ', jstr(stacks[stacks.length-1]));
			...
			console.log('elementNode endTag :: cursor ==> ', cursor);
        } else {
			console.log('elementNode throw :: cursor, text, input[cursor], stack, stacks \n', cursor, ', ', text, ', ', input[cursor], ', ', jstr(stack), ', ', jstr(stacks));
            throw "처리 불가능한 노드";
        }
        console.log('elementNode res :: cursor, text, input[cursor], stack, stacks \n', cursor, ', ', text, ', ', input[cursor], ', ', jstr(stack), ', ', jstr(stacks));
    } else {
		console.log('elementNode no \'<\' :: cursor, text, input[cursor]', cursor, ', ', text, ', ', ', ', input[cursor]);
		text += char;
    }
